...
from pynegative.core import de_noise_image
...
def main():
    ...
    # --- Benchmarking Suite ---
    methods = [
        ("High Quality", 3),             # Bilateral Reference
        ("NLMeans (Numba Y)", 1),        # Luminance Only
        ("NLMeans (Numba)", 1),          # Full YUV (Slowest)
        ("NLMeans (Numba Fast)", 3),     # Fast YUV (Reduced windows)
        ("NLMeans (Numba Fast+)", 5),    # Fast+ YUV (Even smaller)
    ]

    results = {}
    for method, iters in methods:
        results[method] = benchmark_method(noisy_img, method, strength, iterations=iters)
    ...
    cv2.putText(h_stack, "NLM Fast", (size[1]*2 + 10, 30), font, 1, (255, 255, 255), 2)
    cv2.putText(h_stack, "NLM Fast+", (size[1]*3 + 10, 30), font, 1, (255, 255, 255), 2)
    cv2.putText(h_stack, "NLM Full", (size[1]*4 + 10, 30), font, 1, (255, 255, 255), 2)
