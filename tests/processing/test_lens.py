...
def test_lens_map_cache():
    ...
    map_x2, map_y2 = cache.get_maps(w, h, "ptlens", {"a": 0.01, "b": 0.02, "c": 0.03}, cx, cy, fw, fh, 1.0)
...
def test_remap_tca_distortion_kernel():
    ...
    remap_tca_distortion_kernel(img, out, 1, dist_p, t_red, t_blue, 5.0, 5.0, 10, 10, 1.0)
...
def test_calculate_autocrop_scale():
    ...
    assert scale >= 1.0
    scale_pt = calculate_autocrop_scale("ptlens", {"a": -0.01, "b": -0.01, "c": -0.01}, 100, 100)
    assert scale_pt >= 1.0
...
def test_generate_tca_maps():
    maps = generate_tca_maps(10, 10, 1, np.array([0.1], dtype=np.float32), np.array([1.0, 0.0, 0.0], dtype=np.float32), np.array([1.0, 0.0, 0.0], dtype=np.float32), 5.0, 5.0, 10, 10, 1.0)
    assert len(maps) == 6

def test_get_tca_distortion_maps():
    settings = {"lens_distortion": 0.2, "lens_ca": 1.0, "lens_autocrop": True}
    lens_info = {"tca": {"vr0": 1.001, "vb0": 0.999}, "distortion": {"model": "poly3", "k1": -0.1}}
...
def test_get_lens_distortion_maps():
    ...
    mx, my, zoom = get_lens_distortion_maps(10, 10, settings, lens_info, roi_offset=(0, 0), full_size=(10, 10))
    assert mx is not None

def test_apply_lens_correction():
    img = np.ones((10, 10, 3), dtype=np.float32)
    settings = {"lens_distortion": -0.1, "lens_vignette": -0.1, "lens_ca": 1.0, "lens_autocrop": True}
    lens_info = {
